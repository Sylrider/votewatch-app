# WatchGov ÃÂ¢ÃÂÃÂ PROJECT STATUS (Shared Source of Truth)

> ÃÂ¢ÃÂÃÂ ÃÂ¯ÃÂ¸ÃÂ **MANDATORY PROTOCOL FOR EVERY CLAUDE SESSION (Chrome extension, Desktop, web ÃÂ¢ÃÂÃÂ all of them):**
>
> 1. **BEFORE doing ANY work on WatchGov:** read this entire file from the repo first.
>    (`github.com/Sylrider/votewatch-app` ÃÂ¢ÃÂÃÂ `PROJECT_STATUS.md`). Do not act on stale
>    memory ÃÂ¢ÃÂÃÂ this file is the only current truth.
> 2. **AFTER doing ANY work:** update this file ÃÂ¢ÃÂÃÂ tick items in ÃÂÃÂ§12, add a dated line to
>    the ÃÂÃÂ§13 changelog, update the "Last updated" stamp ÃÂ¢ÃÂÃÂ and commit/push it to the repo
...
>
> Last updated: 2026-05-29 ÃÂ¢ÃÂÃÂ by: main chat session
...
flagged), lawsuits/ethics issues, an "Independent View" summary, and a 0ÃÂ¢ÃÂÃÂ100 Transparency
...
- **Host:** Cloudflare Pages ÃÂ¢ÃÂÃÂ auto-deploys on every push via Cloudflare's NATIVE GitHub
  integration (NOT the GitHub Actions workflow ÃÂ¢ÃÂÃÂ see ÃÂÃÂ§6).
...
## 3. THE GOAL ROSTER (~900ÃÂ¢ÃÂÃÂ950 officials, 5 categories)
...
|------------|---------|----------------------------------------------------------|
| Executive  | ~handful| President, VP, Cabinet ÃÂ¢ÃÂÃÂ manual + FEC for federal money   |
| Senate     | 100     | FULLY automatable via federal APIs                        |
| House      | 435     | FULLY automatable via federal APIs                        |
| Governor   | 50      | NO unified free API ÃÂ¢ÃÂÃÂ hand-curate / per-state portals     |
| Mayor      | ~335    | Mayors of cities >100k pop. NO free API ÃÂ¢ÃÂÃÂ hand-curate     |

**Key nuance:** Free APIs only fully cover FEDERAL officials (Executive/Senate/House).
Governors & mayors have no unified free data source ÃÂ¢ÃÂÃÂ port from the earlier hand-built
...
vote-alignment component ÃÂ¢ÃÂÃÂ they have no roll-call record).
...
The pipeline maps FEC industry/PAC codes ÃÂ¢ÃÂÃÂ these IDs to link politicians ÃÂ¢ÃÂÃÂ lobbies.

---

## 5. APIS ÃÂ¢ÃÂÃÂ WHAT & HOW
1. **Congress.gov** ÃÂ¢ÃÂÃÂ Senate+House only. Key `CONGRESS_API_KEY`. Base
   ...
   ÃÂ¢ÃÂÃÂ identity, party, state, chamber, committees, bills, votes.
2. **FEC / OpenFEC** ÃÂ¢ÃÂÃÂ federal campaign money. Key `FEC_API_KEY` (api.data.gov). Base
   `https://api.open.fec.gov/v1/`. Endpoints: `/candidates/search`,
   `/candidate/{id}/totals`, `/schedules/schedule_a`, `/committee/{id}`. ~1k/hr ÃÂ¢ÃÂÃÂ CACHE.
   ÃÂ¢ÃÂÃÂ feeds lobbyScore; links donors to lobby IDs.
3. **Senate/House Stock Watcher** ÃÂ¢ÃÂÃÂ Congress only. No key. Full JSON at
   senatestockwatcher.com & housestockwatcher.com. ÃÂ¢ÃÂÃÂ feeds stockScore + conflict flags.
4. **CourtListener** ÃÂ¢ÃÂÃÂ lawsuits, federal + state. No key (optional token). Base
   `https://www.courtlistener.com/api/rest/v4/`, `/search/?q={name}`. ÃÂ¢ÃÂÃÂ feeds legalScore.
5. **OpenSecrets** ÃÂ¢ÃÂÃÂ SKIP v1 (paid/application). Approximate lobby intent from FEC codes.

Pipeline per federal politician: Congress.gov (identity+votes) ÃÂ¢ÃÂÃÂ FEC (money) ÃÂ¢ÃÂÃÂ
Stock Watchers (trades) ÃÂ¢ÃÂÃÂ CourtListener (lawsuits) ÃÂ¢ÃÂÃÂ map donors to lobbies ÃÂ¢ÃÂÃÂ compute
score ÃÂ¢ÃÂÃÂ write record to `data/politicians.json`.
...
## 7. TRANSPARENCY RISK SCORE (0ÃÂ¢ÃÂÃÂ100)
`Total = min(100, lobbyScore + alignScore + stockScore + legalScore)`
- lobbyScore = min(25, totalLobbyMoney / 100,000 ÃÂÃÂ 1.2)
- alignScore = (donorAlignedVotes / totalVotes) ÃÂÃÂ 35   [SKIP for non-federal; reweight]
- stockScore = min(25, conflictTrades ÃÂÃÂ 5 + (10 if any single trade ÃÂ¢ÃÂÃÂ¥ $500K))
- legalScore = min(15, ÃÂÃÂ£ severity: high=7, medium=4, low=1)

Bands: CRITICAL 75ÃÂ¢ÃÂÃÂ100 (#ef4444) ÃÂÃÂ· HIGH 50ÃÂ¢ÃÂÃÂ74 (#f97316) ÃÂÃÂ· ELEVATED 25ÃÂ¢ÃÂÃÂ49 (#eab308) ÃÂÃÂ·
LOW 0ÃÂ¢ÃÂÃÂ24 (#22c55e).
...
## 9. CRITICAL TECH RULES (these caused past build failures ÃÂ¢ÃÂÃÂ DO NOT REPEAT)
...
- No event handlers (onClick/onMouseEnter) in server components ÃÂ¢ÃÂÃÂ use CSS hover.
...
## 10. PAGES
- `app/page.tsx` ÃÂ¢ÃÂÃÂ hero + stats + searchable/filterable grid (client: components/PoliticianGrid.tsx)
- `app/politicians/[slug]/page.tsx` ÃÂ¢ÃÂÃÂ full profile, generateStaticParams from JSON
- `app/lobbies/page.tsx` + `app/lobbies/[slug]/page.tsx` ÃÂ¢ÃÂÃÂ lobby list + detail
- `app/methodology/page.tsx` ÃÂ¢ÃÂÃÂ score formula, bands, sources, disclaimer
...
- **Claude-in-Chrome extension:** can drive the browser ÃÂ¢ÃÂÃÂ edit files on GitHub, click
